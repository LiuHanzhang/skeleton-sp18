...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        while (true) {
            double prio = getNode(index).priority();
            Node l = getNode(leftIndex(index));
            Node r = getNode(rightIndex(index));
            if (l == null || (r == null && prio < l.priority()) ||
                    (r != null && prio < l.priority() && prio < r.priority()))
                break;
            int newIndex;
            if (r == null)
                newIndex = leftIndex(index);
            else
                newIndex = l.priority() < r.priority() ? leftIndex(index) : rightIndex(index);
            swap(index, newIndex);
            index = newIndex;
        }
    }
